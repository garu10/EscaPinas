...
$query = "SELECT * FROM vouchers WHERE user_id = '$user_id' ORDER BY voucher_id DESC";
...
<head>
    ...
    <link rel="stylesheet" href="assets/css/voucher.css"> <style>
        .ticket-card { border: none; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.08)); }
        .border-dashed-start { border-left: 2px dashed #dee2e6 !important; position: relative; }
        .ticket-card::before, .ticket-card::after {
            content: ''; position: absolute; left: 33.33%; width: 16px; height: 16px;
            background: #f8f9fa; border-radius: 50%; z-index: 10; transform: translateX(-50%);
        }
                        ...
                        <div class="row g-0 align-items-stretch">
                            <div class="col-4 p-4 text-center <?php echo $isUsed ? 'bg-secondary' : 'bg-success'; ?> text-white d-flex flex-column justify-content-center">
                                <small class="text-uppercase opacity-75 d-block mb-1" style="font-size: 0.6rem;">
                                    <?php echo htmlspecialchars(str_replace('_', ' ', $v['external_system'])); ?>
                                </small>
