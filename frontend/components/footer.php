<div class="container-fluid" style="background-color: #0CA458;">
    <div class="container pb-4">
        <div class="row text-center text-md-start justify-content-between">
            
            <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mt-5 text-center">
                <div class="mb-3">
                    <img src="/EscaPinas/frontend/assets/images/Logo 1.png" height="150" class="d-inline-block align-text-top">
                </div>
                <p class="small text-light">
                    EscaPinas is your gateway to affordable tours across the Philippines. Explore, enjoy, and create unforgettable memories with every trip.
                </p>
            </div>

            <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mt-5 align-items-center">
                <h6 class="text-uppercase mb-4 fw-bold text-light">Company</h6>
                <ul class="list-unstyled small lh-lg">
                    <li><a href="/EscaPinas/frontend/components/infoLinks/about.php" class="text-light text-decoration-none hover-link">About Us</a></li>
                    <li><a href="/EscaPinas/frontend/components/infoLinks/faq.php" class="text-light text-decoration-none hover-link">Travel FAQs</a></li>
                    <li><a href="/EscaPinas/frontend/components/infoLinks/privacy.php" class="text-light text-decoration-none hover-link">Privacy Policy</a></li>
                    <li><a href="/EscaPinas/frontend/components/infoLinks/terms.php" class="text-light text-decoration-none hover-link">Terms & Conditions</a></li>
                </ul>
            </div>

            <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mt-5">
                <h6 class="text-uppercase mb-4 fw-bold text-light">Contact Us</h6>
                <div class="small text-light lh-lg">
                    <p class="d-flex align-items-center justify-content-center justify-content-md-start mb-2">
                        <i class="bi bi-house-door me-3 fs-5"></i> Tanauan City, Batangas, PH
                    </p>
                    <p class="d-flex align-items-center justify-content-center justify-content-md-start mb-2">
                        <a href="mailto:escapinas26@gmail.com" class="text-light text-decoration-none">
                            <i class="bi bi-envelope me-3 fs-5"></i> escapinas26@gmail.com
                        </a>
                    </p>
                    <p class="d-flex align-items-center justify-content-center justify-content-md-start mb-2">
                        <i class="bi bi-telephone me-3 fs-5"></i> +63 966 693 5239
                    </p>
                </div>
            </div>

        </div>

        <hr class="my-4 border-secondary opacity-25">

        <div class="row align-items-center">
            <div class="col-12 text-center">
                <p class="small text-light p-2 mb-0">
                    © <?php echo date("Y"); ?> <strong>EscaPinas</strong>. All Rights Reserved.
                </p>
            </div>
        </div>
    </div>
</div>